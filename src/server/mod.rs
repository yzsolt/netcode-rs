    ...
    I: SocketProvider<I, S>,
{
    ...
    pub fn send(&mut self, client_id: ClientId, packet: &[u8]) -> Result<usize, SendError> {
        ...
        if let Some(client) = Self::find_client_by_id(&mut self.clients, client_id) {
            trace!(
                "Sending packet to {} with length {}",
                client_id,
                packet.len()
            );
            self.internal
                .send_packet(client, &packet::Packet::Payload(packet.len()), Some(packet))
        } else {
    ...
    pub fn next_event(
        ...
        loop {
            ...
            let event = match result {
                ...
                TickResult::StateChange(state) => match state {
                    ConnectionState::TimedOut | ConnectionState::Disconnected => {
                        let client_id = clients[self.internal.client_event_idx]
                            .as_ref()
                            .map_or(0, |c| c.client_id);
    ...
    fn find_client_by_addr<'a>(
        clients: &'a mut ClientVec,
        addr: &SocketAddr,
    ...
    I: SocketProvider<I, S>,
{
            ...
            existing_client_result.unwrap_or_else(|| {
                ...
                if let Some(idx) = clients.iter().position(|v| v.is_none()) {
                    let mut conn = Connection {
                        ...
                        channel: Channel::new(
                            &private_data.server_to_client_key,
    ...
    fn send_client_challenge(
        &mut self,
        client: &mut Connection,
        private_data: &token::PrivateData,
    ) -> Result<(), UpdateError> {
    ...
    fn send_denied_packet(
        ...
        packet::encode(
            &mut packet[..],
            self.protocol_id,
            &packet::Packet::ConnectionDenied,
            Some((0, key)),
            None,
        )?;
    ...
    fn validate_client_token(
        &mut self,
        req: &packet::ConnectionRequestPacket,
    ) -> Option<token::PrivateData> {
    ...
    fn tick_client(&mut self, client: &mut Connection) -> Result<TickResult, UpdateError> {
        ...
        let result = match *state {
            ConnectionState::PendingResponse => {
                match client
                    .channel
                    .update(self.time, &mut self.listen_socket, false)?
                {
                    ...
                    channel::UpdateResult::SentKeepAlive | channel::UpdateResult::Noop => {
                        TickResult::Noop
                    }
                }
            }
            ConnectionState::Idle => {
                match client
                    .channel
                    .update(self.time, &mut self.listen_socket, true)?
                {
                    channel::UpdateResult::Expired => {
                        trace!("Failed to hear from client {}, timed out", client.client_id);
                        TickResult::StateChange(ConnectionState::TimedOut)
        ...
        match packet::decode(data, self.protocol_id, None, payload) {
            Ok(packet) => match packet.1 {
                packet::Packet::ConnectionRequest(req) => {
                    self.handle_client_connect(addr, &req, clients)
                }
                packet => {
                    trace!(
    ...
    fn handle_packet(
        ...
        let event = match client.state {
            ConnectionState::Idle => match decoded {
...
mod test {
        ...
        I: SocketProvider<I, S>,
    {
        ...
        fn read_challenge(&mut self) -> ChallengePacket {
            let mut data = [0; NETCODE_MAX_PACKET_SIZE];
            self.socket
                .set_recv_timeout(Some(Duration::from_secs(15)))
                .unwrap();
        ...
        fn generate_payload_packet(
            &mut self,
            payload: &[u8],
        ) -> (usize, [u8; NETCODE_MAX_PACKET_SIZE]) {
    ...
    /*
     ...
     fn test_capi_payload() {
         ...
         unsafe {
             ...
             let client = netcode_client_create(::std::mem::transmute(addr.as_ptr()), 0.0, sim.borrow_mut().sim);
             //let client = netcode_network_simulator_create(::std::mem::transmute(addr.as_ptr()), 0.0, sim.borrow_mut().sim, std::ptr::null_mut(), None, None);
